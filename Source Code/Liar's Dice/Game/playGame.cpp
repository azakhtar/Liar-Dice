...
playGame::playGame(int player1Type, int player2Type, int dicePerPlayer, int playerTurn, int trainingGames, int realGames){
	startGame(player1Type, player2Type, dicePerPlayer, playerTurn, trainingGames, realGames);
}

/* This function resets defaults */
void playGame::setDefaults(){
	player1Dice = -1;
	player2Dice = -1;
	player1Roll.clear();
	player2Roll.clear();
	currCall = (std::make_tuple(0, 0));
	...
	gamesP2Won = 0;
	winner = 0;
	totalGamesThisIteration = 0;
}

void playGame::startGame(int player1Type, int player2Type, int dicePerPlayer, int playerTurn, int trainingGames, int realGames){

	player player1(player1Type);
	player player2(player2Type);
	turn = playerTurn;

	/* The entire code is ran twice. TrainingComplete is set to 0. When 0, trainingComplete sets
	 * total games in that iteration to trainingGames. Once training is complete, trainingComplete
	 * is set to 1, indicating training is done. Now total games is set to real games and the
	 * entire while loop underneath is executed again to play the game with trained data.*/
	while ( trainingComplete < 2 ){
		cout << "############################" << endl;

		if ( trainingComplete == 0 ){
			totalGamesThisIteration = trainingGames;
			cout << "RUNNING TRAINING GAMES" << endl;
		}
		else{
		...
		/* Increment trainingComplete and reset turn to playerTurn passed in. Also set default values */
		trainingComplete++;
		turn = playerTurn;
		setDefaults();
	}
